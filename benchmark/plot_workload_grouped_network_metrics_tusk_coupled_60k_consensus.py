#!/usr/bin/env python3
"""
Plot consensus latency and TPS for tusk_coupled at 60k offered load,
grouped by workload with network variants inside each workload group.
"""
...
OUTPUT_PATHS = [
    DATA_ROOT / "consensus" / "workload_grouped_network_metrics_tusk_coupled_60k_consensus.pdf",
    DATA_ROOT / "consensus" / "workload_grouped_network_metrics_tusk_coupled_60k_consensus.png",
]
TARGET_RATE = 60_000
...
def _collect_points(network_dir: str, workload_dir: str) -> list[SummaryPoint]:
    ...
    summary_name = f"{network_dir}_{workload_dir}_summary.txt"
    points = []
...
def plot_combo_metrics(output_paths: list[Path]):
    ...
    ax_tps.set_xlabel("Coupled Architecture at 60k Offered Load", labelpad=2)
