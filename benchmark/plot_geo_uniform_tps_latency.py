#!/usr/bin/env python3
"""
Plot a TPS-vs-latency curve from geo_uniform balanced benchmark summary files.

The script:
- scans `results/geo_uniform/balanced/**/*summary.txt`
...
RESULTS_ROOT = Path("results/geo_uniform/balanced")
MAX_RATE = 50_000
OUTPUT_NAME = "tps_latency_geo_uniform_balanced_upto_50k.png"
USE_END_TO_END = True
...
def _iter_summary_files(root: Path):
    for path in sorted(root.glob("**/geo_uniform_balanced_summary.txt")):
        yield path
...
def plot_tps_latency(points: list[SummaryPoint], output_path: Path):
    ...
    ax.set_title(f"Geo-Uniform Balanced Latency vs TPS ({metric_name})")
