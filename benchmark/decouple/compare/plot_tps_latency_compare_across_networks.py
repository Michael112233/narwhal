#!/usr/bin/env python3
"""
Plot cross-network TPS-latency comparisons for decoupled runs.

Each figure fixes one workload and compares 80ms / geo / geo_uniform across
available offered loads. Outputs are saved under:
- result_decouple/compare/
"""
...
def plot_workload(workload_spec: dict):
    fig, ax = plt.subplots(figsize=(7.0, 4.8))
    series_rows = []

    for network in NETWORKS:
        points = _collect_latest_valid_points(network["dir_name"], workload_spec["dir_name"])
        ...
        ax.errorbar(
            ...
            fmt=f"-{network['marker']}",
            linewidth=2.0,
            markersize=6,
            capsize=3.5,
            color=network["color"],
            ecolor=network["color"],
            label=network["label"],
        )
        ...
        for row in aggregated:
            ax.annotate(
                ...
                xytext=(network["annotate_dx"], 5),
                fontsize=8,
                color=network["color"],
            )

        series_rows.append((network["label"], aggregated))

    ax.set_title(f"{workload_spec['label']} Cross-network Comparison", pad=8)
    ...
    output_stem = OUTPUT_DIR / (
        f"tps_latency_compare_80ms_geo_geo_uniform_{workload_spec['dir_name']}"
    )
...
def main():
    ...
    for workload_spec in WORKLOADS:
        saved_paths, series_rows = plot_workload(workload_spec)
        for output_path in saved_paths:
            print(f"Saved cross-network TPS-latency plot to: {output_path}")
        for label, rows in series_rows:
            for row in rows:
                print(
                    f"  {workload_spec['label']} / {label}: "
