#!/usr/bin/env python3
"""
Plot cross-network certificate collection progress comparisons for decoupled
runs at a fixed offered load.
...
def plot_workload(workload_spec: dict):
    ...
    for network in NETWORKS:
        csv_paths = _collect_latest_csvs(network["dir_name"], workload_spec["dir_name"])
        ...
        ax.plot(
            ...
            markersize=5.5,
            color=network["color"],
            label=f"{network['label']} ({curve['completed_rounds']} rounds)",
        )
        curve_summaries.append((network["label"], curve))

    ax.set_title(
        f"{workload_spec['label']} Certificate Collection Progress",
        pad=8,
    )
    ...
    output_stem = OUTPUT_DIR / (
        f"cert_progress_compare_80ms_geo_geo_uniform_{workload_spec['dir_name']}"
        f"_node{NODE_ID}_rounds_{START_ROUND}_{END_ROUND}"
    )
    fig.tight_layout()
...
def main():
    ...
    for workload_spec in WORKLOADS:
        saved_paths, curve_summaries = plot_workload(workload_spec)
        for output_path in saved_paths:
            print(f"Saved cross-network cert-progress plot to: {output_path}")
