...
class Committee:
    ...
    def __init__(self, addresses, base_port, solid_step_length, solid_step_number):
...
class LocalCommittee(Committee):
    def __init__(self, names, port, workers, solid_step_length, solid_step_number):
        assert isinstance(names, list)
        assert all(isinstance(x, str) for x in names)
        assert isinstance(port, int)
        assert isinstance(workers, int) and workers > 0
        addresses = OrderedDict((x, ['127.0.0.1']*(1+workers)) for x in names)
        super().__init__(addresses, port, solid_step_length, solid_step_number)
