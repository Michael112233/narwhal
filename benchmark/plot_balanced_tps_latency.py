#!/usr/bin/env python3
"""
Plot a latency-vs-TPS curve from balanced benchmark summary files.

The script:
- scans `results/geo/balanced/**/*summary.txt`
- ignores directories ending with `_old`
- ignores rates above `MAX_RATE`
...
RESULTS_ROOT = Path("results/geo/balanced")
MAX_RATE = 50_000
OUTPUT_NAME = "tps_latency_balanced_old_batch_upto_50k.png"
USE_END_TO_END = True
...
def _iter_summary_files(root: Path):
    for path in sorted(root.glob("**/geo_balanced_summary.txt")):
        yield path
...
def _is_old_path(path: Path) -> bool:
    return any(part.endswith("_old") for part in path.parts)
...
def _collect_latest_valid_points(root: Path) -> tuple[list[SummaryPoint], list[str]]:
    ...
    preferred_old_rates = {40_000, 50_000}
    ...
    for rate in sorted({point.rate for point in all_points}):
        ...
        if rate in preferred_old_rates:
            ...
            if old_points:
                rate_points = old_points
            else:
                rate_points = [
                    point
                    for point in rate_points
                    if not _is_old_path(point.path)
                ]
        else:
            rate_points = [
                point
        ...
        aggregated.append(
            {
                ...
                "paths": [str(point.path) for point in runs],
            }
        )
    return aggregated
...
def plot_tps_latency(points: list[SummaryPoint], output_path: Path):
    ...
    # Plot individual valid runs as light reference points.
    ax.scatter(
    ...
    ax.set_title(f"Balanced Latency vs TPS ({metric_name}, old batch up to 50k)")
