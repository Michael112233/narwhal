#!/usr/bin/env python3
"""
Overlay geo and geo_uniform balanced TPS-latency curves in one figure.
...
OUTPUT_PATH = Path("results") / "tps_latency_geo_vs_geo_uniform_balanced_upto_50k.png"
...
def main():
    ...
    if not geo_rows or not geo_uniform_rows:
        raise ValueError("Missing valid points for geo or geo_uniform.")

    OUTPUT_PATH.parent.mkdir(parents=True, exist_ok=True)
    fig, ax = plt.subplots(figsize=(9.0, 5.8))

    ax.scatter(
        [point.tps for point in geo_points],
        [point.latency_ms for point in geo_points],
        color="#1f77b4",
        alpha=0.18,
        s=32,
    )
    ax.scatter(
        [point.tps for point in geo_uniform_points],
        [point.latency_ms for point in geo_uniform_points],
        color="#d95f02",
        alpha=0.18,
        s=32,
    )
    ...
    ax.set_title("Geo vs Geo-Uniform Balanced Latency vs TPS")
    ...
    skipped = geo_skipped + geo_uniform_skipped
    if skipped:
